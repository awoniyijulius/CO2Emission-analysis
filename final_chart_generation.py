#!/usr/bin/env python
"""Direct chart generation with proper imports and error handling"""
...
print("\n" + "=" * 80)
print("SUSTAINABILITY ANALYSIS - CHART GENERATION")
print("=" * 80 + "\n")
...
try:
    ...
    print(f"   H1 (Pearson): r={pearson_r:.4f}, p={pearson_p:.6f}")
    print(f"   H3 (t-test): t={t_statistic:.4f}, p={ttest_p:.6f}")
    print(f"   Outliers found: {len(outliers)}")
    
    # Check existing charts
    print("[3/3] Generating missing charts...")
    
    charts = {
        "05_Outliers_and_Clutch_Moments.png": False,
        "06_Summary_Statistics_Dashboard.png": False
    }
    
    for chart in charts:
        if os.path.exists(chart):
            charts[chart] = True
            print(f"   [SKIP] {chart} already exists")
    
    # Generate 05
    if not charts["05_Outliers_and_Clutch_Moments.png"]:
        print(f"   [CREATE] 05_Outliers_and_Clutch_Moments.png")
    ...
    # Generate 06
    if not charts["06_Summary_Statistics_Dashboard.png"]:
        print(f"   [CREATE] 06_Summary_Statistics_Dashboard.png")
        ...
        ax2.pie([r_squared*100, (1-r_squared)*100], labels=[f'Explained\n{r_squared*100:.1f}%', f'Unexplained\n{(1-r_squared)*100:.1f}%'],
            colors=['lightgreen', 'lightcoral'], startangle=90, textprops={'fontsize': 10, 'fontweight': 'bold'})
    ...
    print("\n" + "=" * 80)
    print("CHART VERIFICATION - FINAL STATUS")
    print("=" * 80)
    ...
    for i, chart in enumerate(EXPECTED_CHARTS, 1):
        status = "[OK]" if os.path.exists(chart) else "[MISSING]"
        print(f"  [{i}] {status} {chart}")
        if os.path.exists(chart):
            completed += 1
    
    print("\n" + "=" * 80)
    print(f"COMPLETE: {completed}/{len(EXPECTED_CHARTS)} visualizations ready")
    print("=" * 80 + "\n")
    
    if completed == len(EXPECTED_CHARTS):
        print("SUCCESS! All 6 charts have been generated.")
        sys.exit(0)
    else:
        print(f"WARNING: {len(EXPECTED_CHARTS) - completed} chart(s) still missing")
        sys.exit(1)
